...
def todo_list():
    ...
    match show:
        ...
        case _:
            return template("message.tpl", message = "Wrong query parameter: show must either be open, closed or all.")
...
def new_task():
    if request.POST:
        ...
        with sqlite3.connect("todo.db") as connection:
            ...
            return template("message.tpl", message = f"The new task was inserted into the database, the ID is {new_id}")
    else:
...
def edit_task(number):
    if request.POST:
        ...
        return template("message.tpl", message = f"The task number {number} was successfully updated")
    else:
        with sqlite3.connect("todo.db") as connection:
            cursor = connection.cursor()
